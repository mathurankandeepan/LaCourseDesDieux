...
public class MenuDroite extends JPanel{
  ...
  private void compteDifferentAcc(final Avatar avatar){
    ...
    livremagique = 0;
    epee = 0;
    final ArrayList<Acc> tab = avatar.getAcc();
    for (final Acc acc : tab){
  ...
  private void compteDifferentAcc(final Sac soussac){
    for (final Acc acc : soussac.getTab()){
      if (acc instanceof Sac){
        compteDifferentAcc((Sac) acc);
        sac++;
        continue;
  ...
  private void dessinerCreature( final Graphics g, final Avatar joueur, final int taille, final int y){
    final ArrayList<Creature> tab=joueur.getAmis();
    int i = 0 ;
    int espace=14 ;
    for ( final Creature creature : tab ) {
  ...
  public void paintComponent(final Graphics g){
    final Font font = new Font("Bookman Old Style", Font.ITALIC, 35);
    final Font font1 = new Font ("Trajan", Font.ITALIC, 15);
    final Font font2 = new Font ("Trajan", Font.ITALIC, 8);
    final Font font3 = new Font ("Arial", Font.ITALIC,25);
    g.setFont(font);
    ...
    final Avatar[] players = Jeu.getPlayers();
